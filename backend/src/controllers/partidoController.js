/**
 * partidoController.js — Controlador de partidos.
 * Listado, creación, finalización con anotaciones, actualización y eliminación de partidos.
 */
const pool = require('../config/db');

/** Lista partidos con datos de equipos. Soporta filtros opcionales: ?temporada_id=X&jornada=Y */
const obtenerTodosLosPartidos = async (req, res) => {
    ...
    try {
        ...
        if (temporada_id) {
            values.push(temporada_id);
            conditions.push(`p.temporada_id = $${values.length}`);
        }

        if (jornada) {
            values.push(jornada);
            conditions.push(`p.jornada = $${values.length}`);
        }
        ...
        query += ` ORDER BY p.jornada ASC, p.fecha ASC, p.horario ASC`;
...
/** Obtiene partidos de una temporada y jornada específicas con nombres y logos de equipos. */
...
const crearPartido = async (req, res) => {
    ...
    try {
        ...
        // Validar que la fecha esté dentro del rango de la temporada
        ...
        // Verificar que ambos equipos estén inscritos en la temporada
...
/**
 * Finaliza un partido: registra marcador, actualiza puntos en la tabla
 * y guarda anotaciones individuales (pichichi). Usa transacción.
 */
const finalizarPartido = async (req, res) => {
    ...
    try {
        ...
        // Sumar puntos: 3 al ganador, o 1 a cada uno si empate
        if (idGanador) {
            await client.query(
                'UPDATE temporada_equipos SET puntos_totales = puntos_totales + 3 WHERE equipo_id = $1 AND temporada_id = $2',
                [idGanador, partido.temporada_id]
            );
        } else {
            await client.query(
                'UPDATE temporada_equipos SET puntos_totales = puntos_totales + 1 WHERE equipo_id IN ($1, $2) AND temporada_id = $3',
                [partido.id_equipo_local, partido.id_equipo_visitante, partido.temporada_id]
            );
        }

        // Guardar anotaciones individuales de jugadores (pichichi)
        if (anotaciones && anotaciones.length > 0) {
            const jugadoresIds = anotaciones.map(n => n.jugador_id);

            const jugadoresRes = await client.query(
                `SELECT id, equipo_id FROM jugadores WHERE id = ANY($1)`,
                [jugadoresIds]
            );

            // Mapa jugador_id → equipo_id para validar sumas
            const jugadorEquipoMap = {};
            jugadoresRes.rows.forEach(j => {
                jugadorEquipoMap[j.id] = j.equipo_id;
            });

            // Sumar puntos por equipo para validar contra el marcador
            let sumaLocal = 0;
            let sumaVisitante = 0;

            for (const nota of anotaciones) {
                const equipoJugador = jugadorEquipoMap[nota.jugador_id];
                if (equipoJugador === partido.id_equipo_local) {
                    sumaLocal += nota.puntos;
                } else if (equipoJugador === partido.id_equipo_visitante) {
                    sumaVisitante += nota.puntos;
                }
            }
            ...
            // Limpiar anotaciones previas (evita duplicados si se re-finaliza)
            await client.query('DELETE FROM anotaciones WHERE id_partido = $1', [id]);

            // Insertar anotaciones individuales
            for (const nota of anotaciones) {
                if (!jugadorEquipoMap[nota.jugador_id]) {
                    throw new Error(`Jugador ${nota.jugador_id} no existe o no tiene equipo`);
                }
                await client.query(
...
/** Actualiza todos los campos editables de un partido. */
...
/** Lista todos los partidos de una temporada con nombres de equipos, ordenados por fecha DESC. */
...
/** Devuelve las jornadas disponibles para una temporada (distintas y > 0). */
